...
레거시 시스템의 일부 기능만 대체하는 작은 규모로 시작한 프로젝트를 진행하였는데 기존 기능 전체를 대체하고 추가 기능까지 지속적으로 업데이트해야 하는 큰 규모로 바뀌었다. 기존 구조로는 확장성과 유지보수성의 한계를 느꼈고 파일 배치 기준과 기능 경계를 명확히 하기 위해 **FSD(Feature-Sliced Design) 구조로 리팩토링**하게 되었다.
...
결국 “기능 단위로 잘 묶이고, 변경 영향 범위가 예측 가능한 구조”가 필요하다.

---

## FSD 폴더 구조

FSD는 파일을 “기능 단위”로 나누고, 레이어별 책임을 분리하는 방식이다.
...
> 핵심은 “기능(features)과 도메인(entities)을 분리하고, 조합은 widgets/pages에서 한다”
...
**entities는 데이터/모델 중심, features는 사용자 행동 중심**으로 정리했다.
...
FSD 폴더 구조를 도입하면서 느낀 장점은 **변경 영향 범위가 예측이 가능**해진 점과 기능 수정 시 관련 코드가 한 곳에 모여 있어 탐색 시간이 줄고 공용(shared) 코드가 무분별하게 커지는 걸 막을 수 있었다.
FSD 기본 구조의 틀에 맞추기보단 **프로젝트에 맞도록 유연하게 기준을 정하고 지속적으로 유지**하는게 중요하다고 느꼈다.
